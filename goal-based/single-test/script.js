		...
		_renderPDF: function(data, rootElement) {
			...
			questions.forEach(function(question, index) {
				...
				if (index === questions.length - 1) {
					// If this is the last question, check for forced new page
		...
		_placeQuestion: function(root, pagesState, q) {
			...
			return new Promise(function(resolve) {
				...
				const tryPlacement = function() {
					...
					// If previous scaled question was in the right column, force a new page and start at left
					if (pagesState.forceNewPageNext) {
						const newIsOdd = ((pagesState.pageCount + 1) % 2) === 1;
						const newPage = self._createNormalPage(newIsOdd, pagesState.data);
						root.appendChild(newPage);
					...
					const lessonName = pagesState.data.lessonName || '';
					if (lessonName.toLowerCase().includes('matematik')) {
